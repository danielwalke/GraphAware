...
"""
Access logs via
awk '/PRINT_GCN_LOGS_START/,/PRINT_GCN_LOGS_END/' file
awk '/PRINT_CHEB_LOGS_START/,/PRINT_CHEB_LOGS_END/' file
"""
...
class Cheb(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim = 121, dropout = .2, normalization = False, K = 2):
        super(Cheb, self).__init__()
        hidden_dim = int(hidden_dim)
        K = int(K)
        self.conv1 = ChebConv(in_dim, hidden_dim, K = K, normalization = normalization)
        self.conv2 = ChebConv(hidden_dim, hidden_dim, K = K, normalization = normalization)
        self.conv3 = ChebConv(hidden_dim, out_dim, K = K, normalization = normalization)
        self.lin1 = Linear(in_dim, hidden_dim)
        self.lin2 = Linear(hidden_dim, hidden_dim)
        self.lin3 = Linear(hidden_dim, out_dim)
        self.dropout = nn.Dropout(p=dropout)
...
print("PRINT_GCN_LOGS_END")

cheb_space = ChebSpace(dataset)

gnn_nestedCV_evaluation_cheb = GNNNestedCVEvaluationInductive(device, Cheb,data, max_evals= len(cheb_space.get_space().keys())*20, epochs  = 10_000, PATIENCE=10)
gnn_nestedCV_evaluation_cheb.nested_cross_validate(5,5, cheb_space.get_space())

print("PRINT_CHEB_LOGS_START")
print("Best params")
print(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.best_params_per_fold)

print("Outer scores")
print(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.outer_scores)
print(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.outer_scores.mean())
print(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.outer_scores.std())

print("Inner scores")
print(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.inner_scores.mean())
print(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.inner_scores.std())

print("Train times")
print(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.train_times)
print(np.array(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.train_times).mean())
print(np.array(gnn_nestedCV_evaluation_cheb.nested_inductive_cv.train_times).std())
print("PRINT_CHEB_LOGS_END")
